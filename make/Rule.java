...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new IllegalStateException("error: empty command set");
        } else {
    ...
    private void checkFinishedDependencies() {
        Iterator<Integer> it = _depends.successors(getVertex());
        while (it.hasNext()) {
            int v = it.next();
    ...
    private boolean outOfDate() {
        ...
        while (it.hasNext()) {
            int i = it.next();
            Rule rule = _depends.getLabel(i);
            Integer t1 = getTime();
            Integer t2 = rule.getTime();
            if (t1 < t2 || t2 == null) {
                return true;
            }
        }
